/**
 * /buyers — buyer identity profiles (M3 BuyerRegistry).
 *
 * List/search read the indexed `buyers` read model; detail is DB-first with an
 * on-chain `profileOf` fallback so a buyer registered but not yet indexed still
 * resolves.
 */
import { z } from 'zod';
...
export default defineRoutes((app, ctx) => {
  ...
  app.get('/buyers/search', async (request) => {
    const { q, org_id } = parseOr400(SearchQuery, request.query);
    const pagination = parsePagination(request.query);
    const filters = org_id !== undefined ? { org_id } : undefined;
    const rows = await ctx.db.list<BuyerRow>(TABLE, {
      ...pagination,
      order: { column: 'created_at', ascending: false },
      filters,
    });
  ...
  app.get('/buyers/:address', async (request) => {
    const { address } = parseOr400(AddressParams, request.params);
    const row = await ctx.db.getBy<BuyerRow>(TABLE, 'address', address);
    if (row !== null) return ok({ ...row, source: 'db' as const });

    const onChain = await readChainProfile(ctx, address);
    if (onChain === null) {
      throw notFound(`Buyer ${address} not found`);
    }
    return ok({ ...(jsonSafe(onChain) as BuyerRow), source: 'chain' as const });
  });
});
