 ...
 *
 * List/search are served from the indexed `suppliers` read model; detail is
 * DB-first and falls back to an on-chain `profileOf` read when the row has not
 * been indexed yet, so a freshly-registered supplier is still resolvable.
 */
import { z } from 'zod';
...
export default defineRoutes((app, ctx) => {
  ...
  app.get('/suppliers/:address', async (request) => {
    ...
    return ok({ ...(jsonSafe(onChain) as SupplierRow), source: 'chain' as const });
  });
});
